...
const props = defineProps(['task','extra_data'])
...
<template>
  <div class="rounded-2xl bg-white p-4 w-full" @click="navigateTo(`/task/${task.id}`)">
    ...
    <div class="flex items-center gap-3 mb-4">
      <div class="border rounded-xl px-3 py-2 grow">
        ...
        <p class="text-primary font-medium text-xs flex gap-2 items-center"><svg width="10" height="10" viewBox="0 0 10 10" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          {{task.price}} ({{parseFloat((task.price / 1000) * 50).toFixed(2)}}$)
        </p>
      </div>
      <div class="border rounded-xl px-3 py-2 grow">
      ...
      <div class="border rounded-xl px-3 py-2 grow">
        <p class="text-gray-400 font-medium text-xs mb-1">Кол-во</p>
    ...
    <div v-if="extra_data">
          <p v-if="extra_data.need_extra_work" class="text-white text-xs bg-orange-700 px-2 py-1 rounded-2xl inline-block">Нужна доработка</p>
          <p v-if="extra_data.decline" class="text-white text-xs bg-red-600 px-2 py-1 rounded-2xl inline-block">Отменено</p>
    </div>
