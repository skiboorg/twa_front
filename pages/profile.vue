...
<div class="container  ">
  ...
  <div class="flex items-center justify-center flex-col mb-4">
    ...
    <div class="flex items-center justify-center gap-4">
      ...
      <p class="bg-white rounded-2xl items-center p-2 flex gap-2 text-sm text-primary">
        ...
        {{user.balance}} ({{parseFloat((user.balance / 1000) * 50).toFixed(2)}}$)
      </p>
    </div>
  </div>

  <p class="font-medium text-left mb-3">Рейтинг пользователя</p>
  <div class="bg-white p-4 rounded-2xl mb-3">
    <p class="opacity-50 text-xs">
      Рейтинг варьируется от минимального (1.0) до максимального (5.0) и высчитывается по среднему значению ваших результатов. Если ваш рейтинг опускается ниже 4.0 - вы ограничиваетесь 2 задачами, если ниже 3.0 - вы находитесь на ручной модерации.<br>
      В случае 2 и более подряд просрочек или отказов мы имеем право заблокировать вам доступ к работе.<br>
      Успешное выполнение задачи = 5.0<br>
      Просрочка на выполнение от 1 до 6 часов = 4.5<br>
      Просрочка на выполнение от 12 до 24 часов = 3.0<br>
      Просрочка на выполнение от 24 часов = 2.0<br>
      Отказ от выполнения задачи через 12 и более часов после взятия задачи = 4.0<br>
      Отказ от выполнения задачи перед окончаем срока выполнения = 1.0
    </p>
  </div>
  ...
  <div class="bg-white p-4 rounded-2xl mb-3">
    ...
    <p class="opacity-50 text-xs">
      Вывод средств осуществляется на указанный вами ваш TON кошелек по актуальному на момент вывода курсу TON к $.<br>
      После запроса на вывод ваша заявка будет отображена ниже.<br>
      Постоянный курс 1000 Coin = 50$
    </p>
  </div>
  <p class="font-medium mb-2">Вывод средств</p>
  <div class="bg-white p-4 rounded-2xl mb-3">

    <Select fluid class="mb-2" v-model="amount" :options="[1000,2000,3000,5000,10000]" placeholder="Выберите сумму вывода"/>

    <Button v-if="user.wallet" fluid :disabled="!amount"  label="Отправить заявку" @click="withdrawal_request(amount)"/>
    <p v-else class="text-center text-sm text-red-500">Укажите адрес кошлька для вывода</p>
  </div>

  <p class="font-medium text-left mb-3">Запросы на вывод</p>
  <div class="bg-white p-4 rounded-2xl mb-3 ">
    <div class="border-b mb-1 pb-1" v-for="item in user.requests" :key="item.id">
      <p class="text-xs opacity-50 ">{{new Date(item.created_at).toLocaleString()}}</p>
      <p class="bg-white rounded-2xl items-center  flex gap-2 text-sm text-primary">
        <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path fill-rule="evenodd" clip-rule="evenodd" d="M7.13869 12.1902L4.00621 14.1092C3.68049 14.3087 3.25468 14.2064 3.05515 13.8807C2.95769 13.7216 2.92864 13.5299 2.97457 13.3491L3.45948 11.4405C3.63452 10.7515 4.10599 10.1756 4.74682 9.86791L8.1642 8.22716C8.32352 8.15067 8.39067 7.95951 8.31418 7.80019C8.25223 7.67116 8.11214 7.59896 7.97111 7.62338L4.16713 8.28194C3.39387 8.41581 2.6009 8.20228 1.99941 7.69823L0.797703 6.69116C0.504936 6.44581 0.466494 6.00958 0.711842 5.71682C0.83117 5.57442 1.00277 5.48602 1.18798 5.47153L4.85956 5.18419C5.11895 5.16389 5.345 4.99974 5.44458 4.75937L6.86101 1.3402C7.0072 0.987302 7.41179 0.819734 7.76469 0.965925C7.93413 1.03612 8.06876 1.17075 8.13896 1.3402L9.55539 4.75937C9.65496 4.99974 9.88101 5.16389 10.1404 5.18419L13.8322 5.47311C14.213 5.50291 14.4975 5.83578 14.4677 6.2166C14.4534 6.39979 14.3667 6.56975 14.2269 6.68896L11.4114 9.08928C11.2131 9.25826 11.1267 9.52425 11.1876 9.77748L12.0532 13.3733C12.1426 13.7447 11.914 14.1182 11.5427 14.2076C11.3642 14.2506 11.176 14.2208 11.0195 14.1249L7.86128 12.1902C7.63956 12.0544 7.3604 12.0544 7.13869 12.1902Z" fill="#6092F1"/>
        </svg>
        {{item.amount}} ({{parseFloat((item.amount / 1000) * 50).toFixed(2)}}$)
      </p>
      <p class="text-xs  inline-block px-2 py-1 rounded-lg mb-1 text-gray-50 " :class="item.is_done ? 'bg-green-400' : 'bg-orange-400'">{{item.is_done ? 'Выведено' : 'В работе'}}</p>
      <p class="text-sm opacity-70 ">{{item.wallet}}</p>
    </div>
